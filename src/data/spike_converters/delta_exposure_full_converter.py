"""
Delta-modulation spike encoder and decoder with exposure on inputs and outputs.
"""
...
class DeltaExposureFullSpikeConverter(SpikeConverter):
    ...
    def encode_y(self, y_data: np.ndarray) -> np.ndarray:
        # y_data -> N x C x freq x time
        # output -> N x exp x C x freq x time
        encoded = np.copy(y_data)
...
if __name__ == "__main__":
    ...
    print("Loaded data")
    # Setup encoder
    encoder = DeltaExposureFullSpikeConverter(threshold=0.1, exposure=6)
    # Convert X
    ...
    decoded_y = np.clip(decoded_y, 0, 1)
